...
def get_data_and_analyze(target_date=None, mode_exclusive=True):
    ...
    p1_nos = [p['no'] for p in p1]; p2 = sorted([p for p in all_analysis if p['no'] not in [s1, s2] + p1_nos], key=lambda x: get_synergy(s2, x['no']), reverse=True)[:2]
...
async def index(request: Request, date: str = None, exclusive: bool = True):
    ...
    <head>
        ...
        <style>
            ...
            .matrix-jackpot { border: 2px solid #fbbf24 !important; box-shadow: 0 0 25px rgba(251, 191, 36, 0.6); animation: jackpot-blink 1s infinite alternate; background: rgba(251, 191, 36, 0.1) !important; }

            @keyframes pulse-gold { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }
            @keyframes jackpot-blink { from { border-color: #fbbf24; } to { border-color: #fff; transform: scale(1.01); } }
    ...
    <body class="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-200 pb-20 text-[12px]">
        <div class="max-w-6xl mx-auto p-4 md:p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 text-white text-center italic font-black">
                <div class="bg-indigo-700 p-6 rounded-[2rem] shadow-xl border-2 border-indigo-400">
                    ...
                    <div class="grid grid-cols-2 gap-4 border-t border-white/10 pt-4"><div><p class="text-[8px] opacity-60">今日累計</p><p class="text-lg">奇 {{ p_day }} 偶</p></div><div class="border-l border-white/10"><p class="text-[8px] text-amber-300 underline underline-offset-2">最近 20 期</p><p class="text-xl font-black text-amber-300">{{ p_20 }}</p></div></div>
                </div>
                <div class="bg-emerald-700 p-6 rounded-[2rem] shadow-xl border-2 border-emerald-400">
                    ...
                    <div class="grid grid-cols-2 gap-4 border-t border-white/10 pt-4"><div><p class="text-[8px] opacity-60">今日累計</p><p class="text-lg">小 {{ s_day }} 大</p></div><div class="border-l border-white/10"><p class="text-[8px] text-amber-300 underline underline-offset-2">最近 20 期</p><p class="text-xl font-black text-amber-300">{{ s_20 }}</p></div></div>
                </div>
            </div>
                ...
                <div class="lg:col-span-3 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 italic">
                    ...
                    <div class="space-y-6 text-center">
                        <div class="bg-rose-50 dark:bg-rose-950/20 p-5 rounded-[2.5rem] border-2 border-rose-100 dark:border-rose-900/30">
                            <p class="text-[10px] font-black text-rose-500 mb-4 uppercase tracking-[0.4em] underline underline-offset-8 italic">🔥 VIP 雙核心強攻狙擊</p>
            ...
            <div class="bg-white dark:bg-slate-900 p-6 rounded-[3rem] shadow-xl border border-slate-200 dark:border-slate-800 mb-6 italic">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div class="space-y-2 text-center font-black border-r dark:border-slate-800 pr-4">
                        <label class="text-[8px] text-slate-400 uppercase tracking-widest">倍數 / 組數</label>
            ...
            <div class="bg-emerald-900 p-8 rounded-[3rem] shadow-2xl text-white mb-10 border-4 border-emerald-800 italic">
                <div class="flex justify-between items-center mb-8 border-b border-emerald-700 pb-4 font-black">
                    <h3 class="text-[11px] uppercase tracking-[0.3em]">今日實戰累積日誌</h3>
                    <button onclick="resetDailyJournal()" class="text-[8px] bg-red-900 px-4 py-1.5 rounded-full uppercase shadow-lg">重置數據</button>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-center font-black">
                    <div class="bg-black/20 p-5 rounded-3xl border border-white/5"><p class="text-[9px] text-emerald-300 uppercase mb-1">今日成本</p><p class="text-2xl font-mono" id="day-total-cost">$ 0</p></div>
                    <div class="bg-black/20 p-5 rounded-3xl border border-white/5"><p class="text-[9px] text-emerald-300 uppercase mb-1">今日獎金</p><p class="text-2xl font-mono" id="day-total-prize">$ 0</p></div>
                    <div class="bg-emerald-500 text-emerald-950 p-5 rounded-3xl shadow-lg border-2 border-emerald-400"><p class="text-[9px] uppercase mb-1">今日純利</p><p class="text-3xl font-mono" id="day-net-profit">$ 0</p></div>
                </div>
            </div>
            ...
            <div class="space-y-8 mb-10 font-black italic font-black">
                <div class="bg-slate-900 p-8 rounded-[3rem] shadow-2xl border-4 border-slate-800 text-white font-black">
                    <div class="flex justify-between items-center mb-6 px-2 text-amber-400 uppercase tracking-widest font-black"><h3 class="text-[10px]">三星矩陣庫</h3><div class="flex gap-2 font-black"><button onclick="loadAll('3s')" class="bg-green-700 text-white px-4 py-1 rounded-md text-[8px] font-black">裝載</button><button onclick="clearMatrix('3s')" class="bg-red-900 text-white px-4 py-1 rounded-md text-[8px] font-black">清除</button></div></div>
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 font-black">{% for i in range(1, 11) %}<div id="set-3s-{{i}}" class="bg-slate-800/50 p-2 rounded-xl border border-transparent text-center font-black"><p class="text-[7px] text-slate-500 uppercase font-black">SET {{ i }}</p><div class="flex gap-1 font-black">{% for j in range(1, 4) %}<input type="text" id="3s-g{{i}}n{{j}}" class="w-full h-8 text-center text-sm font-black bg-slate-900 rounded border border-slate-600 text-white" oninput="saveAndCompare()">{% endfor %}</div></div>{% endfor %}</div>
                </div>
                <div class="bg-slate-900 p-8 rounded-[3rem] shadow-2xl border-4 border-slate-800 text-white font-black">
                    <div class="flex justify-between items-center mb-6 px-2 text-indigo-400 uppercase tracking-widest font-black"><h3 class="text-[10px]">四星矩陣庫</h3><div class="flex gap-2 font-black"><button onclick="loadAll('4s')" class="bg-indigo-700 text-white px-4 py-1 rounded-md text-[8px] font-black">裝載</button><button onclick="clearMatrix('4s')" class="bg-red-900 text-white px-4 py-1 rounded-md text-[8px] font-black">清除</button></div></div>
                    <div class="grid grid-cols-2 md:grid-cols-5 gap-3 font-black">{% for i in range(1, 11) %}<div id="set-4s-{{i}}" class="bg-slate-800/50 p-2 rounded-xl border border-transparent text-center font-black"><p class="text-[7px] text-indigo-300 uppercase font-black">S{{ i }}</p><div class="flex gap-1 font-black">{% for j in range(1, 5) %}<input type="text" id="4s-g{{i}}n{{j}}" class="w-full h-8 text-center text-xs font-black bg-slate-900 rounded border border-slate-600 text-white" oninput="saveAndCompare()">{% endfor %}</div></div>{% endfor %}</div>
                </div>
                <div class="bg-slate-900 p-8 rounded-[3rem] shadow-2xl border-4 border-slate-800 text-white font-black text-center">
                    <h3 class="text-[10px] text-rose-400 uppercase mb-4 underline underline-offset-8">🔥 VIP 六星狙擊中心</h3>
                    <div class="flex justify-center"><div id="set-6s-1" class="bg-slate-800/50 p-6 rounded-[2.5rem] border-2 border-rose-900/50 w-full max-w-xl font-black font-black"><div class="grid grid-cols-6 gap-2 font-black">{% for j in range(1, 7) %}<input type="text" id="6s-g1n{{j}}" class="w-full h-12 text-center text-xl font-black bg-slate-950 rounded-xl border-2 border-slate-700 text-rose-500 focus:border-rose-400 outline-none shadow-inner" oninput="saveAndCompare()">{% endfor %}</div></div></div>
                </div>
            </div>
            ...
            <div class="mb-20">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6 px-2 border-b dark:border-slate-800 pb-4 font-black uppercase italic gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <span class="text-[10px] font-black text-slate-500 dark:text-slate-400">佈陣邏輯：</span>
                        <div class="inline-flex rounded-xl bg-slate-100 dark:bg-slate-800 p-1 shadow-inner border dark:border-slate-700">
                            <button onclick="setExclusive(false)" 
                                class="px-4 py-1.5 rounded-lg text-[10px] font-black transition-all {% if not exclusive %}bg-white dark:bg-slate-600 text-indigo-600 dark:text-white shadow-sm{% else %}text-slate-400 opacity-60{% endif %}">
                                精準集中模式
                            </button>
                            <button onclick="setExclusive(true)" 
                                class="px-4 py-1.5 rounded-lg text-[10px] font-black transition-all {% if exclusive %}bg-indigo-600 text-white shadow-md{% else %}text-slate-400 opacity-60{% endif %}">
                                全域覆蓋策略
                            </button>
                        </div>
                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10 text-[10px] uppercase font-black">
                    {% for sq in res_3star %}<div class="bg-white dark:bg-slate-900 p-3 rounded-2xl shadow-sm border dark:border-slate-800 text-center font-black">G{{ sq.id }} (3S)<div class="flex justify-center gap-1 my-2">{% for n in sq.picks %}<span class="bg-slate-900 text-white px-1.5 rounded font-black">{{ "%02d" | format(n) }}</span>{% endfor %}</div><button onclick='quickFill("3s", {{ sq.id }}, {{ sq.picks | tojson }})' class="w-full bg-amber-50 dark:bg-amber-900/20 text-amber-600 py-1 rounded font-black italic uppercase shadow-sm">裝載</button></div>{% endfor %}
                </div>
                ...
                <div class="bg-white dark:bg-slate-900 p-8 rounded-[3.5rem] shadow-xl border border-slate-200 dark:border-slate-800 text-[10px] font-black italic">
                    <div class="flex flex-col md:flex-row justify-between items-center mb-6 border-b dark:border-slate-800 pb-4 gap-4 uppercase font-black">
                        ...
                        <div class="flex gap-3 font-black">
                            <div id="bt-box-3s" class="bt-miss-normal px-4 py-1.5 rounded-xl text-[9px] font-black">三星未命中: <span id="bt-count-3s">0</span></div>
                            <div id="bt-box-4s" class="bt-miss-normal px-4 py-1.5 rounded-xl text-[9px] font-black">四星未命中: <span id="bt-count-4s">0</span></div>
                            <div id="bt-box-6s" class="bt-miss-normal px-4 py-1.5 rounded-xl text-[9px] font-black">六星未命中: <span id="bt-count-6s">0</span></div>
                        </div>
                    </div>
        ...
        <script>
            ...
            function postToDailyJournal() {
                let log = JSON.parse(localStorage.getItem('bingo_journal_v121') || '{"cost":0, "prize":0}');
                const multi = parseInt(document.getElementById('in-multiplier').value) || 1;
                const sets = parseInt(document.getElementById('in-sets').value) || 10;
                const cost = sets * multi * 25;
                const prize = (
                ...
                localStorage.setItem('bingo_journal_v121', JSON.stringify(log)); updateJournalDisplay(); alert("成功存入日誌！");
            }

            function updateJournalDisplay() {
                const log = JSON.parse(localStorage.getItem('bingo_journal_v121') || '{"cost":0, "prize":0}');
            ...
            function runBacktest() {
                ...
                recentHistory.forEach((draw, idx) => {
                    ...
                    m6S.forEach(sq => { const h = sq.filter(n => draw.nums.includes(n)).length; if(h===3) {pPrize+=25; details.push('<span class="badge-graphical badge-6s opacity-60">6中3</span>');} else if(h===4) {pPrize+=200; details.push('<span class="badge-graphical badge-6s">6中4</span>');} else if(h>=5) {pPrize+=1200; details.push('<span class="badge-graphical badge-jackpot">🏆 6星捷</span>'); hBig6=true;} });
